...
export const BookingPage: React.FC<BookingPageProps> = ({
  ...
  const STEP_NAMES = [
    { num: 1, label: t('booking.steps.problem', 'Problems') },
    { num: 2, label: t('booking.steps.schedule', 'Schedule') },
    { num: 3, label: t('booking.steps.address', 'Address') },
    { num: 4, label: isGenderPrefEnabled ? t('booking.steps.preferences', 'Preferences') : t('booking.steps.details', 'Details') },
    { num: 5, label: t('booking.steps.escrow', 'Escrow') },
  ];

  const selectedAddressObj = savedAddresses.find((a) => a.id === selectedAddressId);

  return (
    <div className="max-w-7xl mx-auto py-4 sm:py-6 space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* LEFT COLUMN: Main Stepper & Form Workflow (8 cols) */}
        <div className="lg:col-span-8 space-y-6">

          {/* Service Header Banner */}
          <div className="bg-white rounded-3xl p-6 sm:p-7 border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="flex items-start gap-4">
              <div className="w-13 h-13 rounded-2xl bg-emerald-700 text-white flex items-center justify-center shrink-0 shadow-md shadow-emerald-900/20">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs space-y-6">
            ...
            {/* STEP 1: PROBLEM SELECTION WITH INDIVIDUAL TASK PRICING */}
            {currentStep === 1 && (
            ...
            {/* STEP 2: SCHEDULE & LATE BOOKING WARNING */}
            {currentStep === 2 && (
              <div className="space-y-6 animate-in fade-in">
                <div>
            ...
            {/* STEP 3: SERVICE ADDRESS (REUSING SAVED ADDRESS BOOK) */}
            {currentStep === 3 && (
              <div className="space-y-6 animate-in fade-in">
            ...
            {/* STEP 4: PREFERENCES & DETAILS (Gender preference controlled by SuperAdmin feature flag) */}
            {currentStep === 4 && (
              <div className="space-y-6 animate-in fade-in">
                <div>
            ...
            {/* STEP 5: ESCROW / PAYMENT CONFIRMATION */}
            {currentStep === 5 && (
              <div className="space-y-6 animate-in fade-in">
                <div>
        ...
        {/* RIGHT COLUMN: Sticky Overview, Order Summary & Service Guarantees (4 cols) */}
        <div className="lg:col-span-4 space-y-4 lg:sticky lg:top-20">

          {/* Live Order & Escrow Summary Card */}
          <div className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-xs space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="text-xs font-extrabold text-slate-900 uppercase tracking-wider">
                Booking Summary
              </div>
              <span className="px-2.5 py-0.5 rounded-md bg-emerald-50 text-emerald-800 text-[10px] font-bold">
            ...
            {/* Service & Price */}
            <div className="space-y-1">
              <div className="text-xs font-bold text-slate-800">{service.nameEn}</div>
              <div className="text-[11px] text-slate-500">{service.durationEst} • {service.category}</div>
            </div>

            {/* Selected Tasks List */}
            {selectedProblems.length > 0 && (
              <div className="space-y-1.5 pt-2 border-t border-slate-100">
                <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Selected Tasks ({selectedProblems.length})</div>
                <div className="space-y-1 max-h-36 overflow-y-auto pr-1">
                  {selectedProblems.map((probId) => {
                    const opt = service.problemOptions.find((p) => p.id === probId);
                    return (
                      <div key={probId} className="flex items-center justify-between text-[11px]">
                        <span className="text-slate-600 truncate pr-2">• {t(opt?.labelKey || probId, probId)}</span>
                        <span className="font-bold text-slate-900 shrink-0">₹{opt?.price || service.baseRate}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {/* Schedule Slot (If configured) */}
            <div className="pt-2 border-t border-slate-100 space-y-1 text-[11px]">
              <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Scheduled Arrival</div>
            ...
            {selectedAddressObj && (
              <div className="pt-2 border-t border-slate-100 space-y-1 text-[11px]">
                <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Service Location</div>
                <div className="flex items-start gap-1.5 text-slate-700">
                  <MapPin className="w-3.5 h-3.5 text-emerald-600 shrink-0 mt-0.5" />
                  <span className="line-clamp-2">{selectedAddressObj.flat}, {selectedAddressObj.area} ({selectedAddressObj.pincode})</span>
                </div>
              </div>
            )}
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
              <div>
                ...
                <span className="text-lg font-black text-emerald-700">₹{currentTotalAmount}</span>
              </div>
              <div className="flex items-center gap-1 text-[10px] text-emerald-800 font-bold bg-emerald-50 px-2 py-1 rounded-lg">
          ...
          {/* Service Guarantees & Scope Card (Moved from Step 1 to Side) */}
          <div className="bg-gradient-to-br from-emerald-50/70 to-teal-50/40 rounded-3xl p-5 border border-emerald-200/80 shadow-xs space-y-3.5">
            <div className="flex items-center gap-2 text-xs font-black text-emerald-950 uppercase tracking-wider">
              <ShieldCheck className="w-4 h-4 text-emerald-700" />
              <span>Cooperative Guarantees</span>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-2.5 text-xs">
                ...
                <div>
                  <div className="font-bold text-slate-900">100% Background Verified</div>
                  <p className="text-[11px] text-slate-600 leading-snug mt-0.5">Artisans verified with Aadhaar and police clearance in your area.</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2.5 text-xs">
                ...
                <div>
                  <div className="font-bold text-slate-900">Transparent Task Pricing</div>
                  <p className="text-[11px] text-slate-600 leading-snug mt-0.5">Pick exactly what needs repair with fixed standard rates and zero markups.</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2.5 text-xs">
                ...
                <div>
                  <div className="font-bold text-slate-900">30-Day Work Warranty</div>
                  <p className="text-[11px] text-slate-600 leading-snug mt-0.5">Free revisit and warranty protection on certified cooperative jobs.</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2.5 text-xs">
                ...
                <div>
                  <div className="font-bold text-slate-900">100% Escrow Protection</div>
                  <p className="text-[11px] text-slate-600 leading-snug mt-0.5">Payment is held safely until you inspect and approve completed work.</p>
                </div>
              </div>
            </div>

            {/* Helpline Notice */}
            <div className="pt-2.5 border-t border-emerald-200/60 flex items-center justify-between text-[11px] text-emerald-900 font-semibold">
              <span>Cooperative Support</span>
              <span className="font-mono font-bold">1800-SAHYOG</span>
            </div>
          </div>
